...
router.post("/savehouseimgs",imageUploader2,function (req,res,next) {
    let url = 'http://libo2020.oicp.io'+"/upload/"+req.files[0].filename
    res.json({"url":url})
});
...
router.post('/saveimg', imageUploader, function(req, res, next) {
    ...
    // res.send("123")
    // res.send(req);
    let url = 'http://libo2020.oicp.io'+"/upload/"+req.file.filename
    res.json({"url":url})
